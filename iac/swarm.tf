...
resource "aws_instance" "swarm-manager" {
  ...
  provisioner "remote-exec" {
    inline = [      
      ...
      "docker node ls",
      "docker node update --label-add manager=yes  ${aws_instance.swarm-manager[count.index].private_ip}"
    ]
...
resource "aws_instance" "swarm-worker" {
  ...
  provisioner "remote-exec" {
    inline = [      
      ...
      "docker node ls",
      "docker node update --label-add worker=yes ${aws_instance.swarm-manager[count.index].private_ip}"
    ]
  }
